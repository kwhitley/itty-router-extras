...
router.get('/:id', withParams, ({ id }) =>
  if (todos[id]) return asJSON(todos[id])
})

// POST to the collection (we'll use async here)
router.post('/todos', withContent, request => asJSON({
  created: 'todo',
  value: request.content,
}))

// 404 for everything else
router.all('*', missingResponse())
...
### 1. Create a Router
```js
import { Router } from 'itty-router-extras'
...
### The Entire Code (for more legibility, [see src on GitHub](https://github.com/kwhitley/itty-router-extras/blob/v1.x/src/itty-router-extras.js))
```js
const Router = (o = {}) =>
  new Proxy(o, {
    get: (t, k, c) => k === 'handle'
