const withCors = (options = {}) => request => {
  ...
  const allowedOrigin = url.origin.match(/[^\w](slick\.af)|(localhost:3000)$/) ? url.origin : 'https://slick.af'
  const corsHeaders = {
...
const addCorsHeaders = (request) => (response) => {
  ...
  if (referer) {
    const url = new URL(referer)
    allowedOrigin = url.origin.match(/[^\w](slick\.af)|(localhost:3000)$/) ? url.origin : allowedOrigin
  }
